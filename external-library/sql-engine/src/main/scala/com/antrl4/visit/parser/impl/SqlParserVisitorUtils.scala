...
object SqlParserVisitorUtils {
  ...
  def getColumnsInfo(cols: mutable.Buffer[CustomSqlParserParser.ColumnUdfStateContext]): Seq[ColumnsInfo] ={
    cols.map(getColumnInfo)
  }

  def getColumnInfo(x: CustomSqlParserParser.ColumnUdfStateContext): ColumnsInfo = {
    {
      getColTransType(x) match {
        ...
        case CONSTANTPARAM_COL =>
          ...
          val colName = if (x.asName == null) consV else x.asName.asColName.getText
          val basicInfo =
        ...
        case UDF_COL =>
          val asColName =
            if (x.asName() != null) x.asName().asColName.getText()
