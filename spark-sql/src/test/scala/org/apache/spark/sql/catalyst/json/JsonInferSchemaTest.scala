...
object JsonInferSchemaTest {
  ...
  def infer[T](json: RDD[T],
            ...
            case e @ (_: RuntimeException | _: JsonProcessingException) =>
              parseMode match {
                case PermissiveMode =>
                  Some(
                    StructType(
                      Seq(StructField(columnNameOfCorruptRecord, StringType))))
    ...
    json.sparkContext.runJob(mergedTypesFromPartitions,
                             foldPartition,
                             mergeResult)
  ...
  /**
    ...
    */
  private def canonicalizeType(tpe: DataType,
                               options: JSONOptions): Option[DataType] =
    tpe match {
      case at: ArrayType =>
  ...
  private def withCorruptField(struct: StructType,
                               other: DataType,
