...
object SpartStreamingTest {
  ...
  def localSparkStream() {
    ...
    var topics = Set("smartadsdeliverylog", "mobileadsdeliverylog")
    var kafkaParams = Map[String, String](
      ...
      "zookeeper.connect" -> zookeeper)
    	  for(topic<-topics){
    		  val dstream = KafkaClusterManager.createDirectStream(ssc, kafkaParams, Set(topic))
    				  dstream.foreachRDD{rdd=>
    				  println(topic+":"+rdd.count)
    				  Thread.sleep(6000)    
    		  }
    	  }
