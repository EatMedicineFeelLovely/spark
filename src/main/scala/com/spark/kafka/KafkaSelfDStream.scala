...
class KafkaSelfDStream[
  ...
  override def compute(validTime: Time): Option[KafkaRDD[K, V, U, T, R]] = {
    var untilOffsets = clamp(latestLeaderOffsets(maxRetries))
    arr.foreach{case(part,endoff)=>
      val tp=new TopicAndPartition("",part)
      val lead=untilOffsets.get(tp).get
      val lo=lead.copy(offset = Math.min(endoff, lead.offset))
      untilOffsets
    }
